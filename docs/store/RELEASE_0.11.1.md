# SignalR Inspector 0.11.1 release handoff
...
- Version: `0.11.1`
- Expected tag: `v0.11.1`
- Release title: `SignalR Inspector 0.11.1`
- Package: `dist/signalr-inspector.zip`
- Package SHA-256: `2dcb9559e9edf311df6dbcec592215c0a77294824ecefe3982fc97f4d4af949a`
...
```text
SignalR Inspector 0.11.1 turns bounded live traces into portable, locally controlled debugging sessions.
...
- Remove transient tab and row IDs, replace browser document IDs with session-local pseudonyms, re-sanitize endpoint tokens, and assign fresh trusted tab, row, and document identifiers during import.
...
2. Confirm `package.json`, `package-lock.json`, and `manifest.json` all report `0.11.1`.
...
6. Create signed tag `v0.11.1` from the verified commit and push it only when publication is
