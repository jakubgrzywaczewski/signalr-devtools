...
- Link the published Medium architecture essay from the project README and synchronize the
  internal article series, promotion schedule, and roadmap with the completed Medium, LinkedIn,
  and X launch posts.
...
- Expose every analysis-package API as a synthetic named export for Node ESM consumers, document
  the supported import form, and exercise named imports from the packed tarball in the release
  gate while preserving CommonJS and browser-script behavior.
...
- Assign the article's deterministic `SendMessage` and Completion records to the active resumed
  connection, and guard their physical connection sequence before rendering the screenshot.
...
- Remove the publication article bundle from the tracked repository and keep it under the ignored
  `docs/internal` authoring area instead. No runtime behavior or permissions changed.
