...
for WebSockets with JSON, HTTP Long Polling with JSON, Server-Sent Events with JSON, and
WebSockets with MessagePack. The clients deliberately expose the SignalR handshake and protocol
frames, making the sample useful when developing or demonstrating SignalR Inspector.
...
Use the separate **WebSockets (JSON)**, **Long Polling (JSON)**, **Server-Sent Events (JSON)**,
and **MessagePack (WebSockets)** buttons to reconnect the page with a specific test scenario. For
Long Polling and the outgoing half of Server-Sent Events, keep DevTools open before selecting the
button or reloading so the DevTools Network observer sees the negotiation response. The
Server-Sent Events client is a hand-rolled, protocol-faithful EventSource transport, so the wire
shapes match a real deployment.
